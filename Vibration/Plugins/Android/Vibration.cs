...
namespace AndroidVibration
{
    public static class Vibration
    {
        ...
        public static int AndroidVersion { get; private set; } // aka API version
        ...
        public static void Init()
        {
            ...
            CanVibrate = DefaultVibrator.VibratorObject.Call<bool>("hasVibrator");
            if (CanVibrate == false)
            {
            ...
            if (AndroidVersion >= 31)
            {
                // there is api support for multiple vibrators, but i haven't seen any discussion online about real world use of this
                int[] vibratorIDs = VibratorManager.GetVibratorIds();
                Vibrator[] vibrators = new Vibrator[vibratorIDs.Length];
                for (int i = 0; i < vibratorIDs.Length; i++)
                {
                    if (DefaultVibrator.id == vibratorIDs[i])
                        vibrators[i] = DefaultVibrator;
                    else
                        vibrators[i] = VibratorManager.GetVibrator(vibratorIDs[i]);
                }
